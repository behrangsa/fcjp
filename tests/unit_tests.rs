...
use fcjp::{process_json_file, FileProcessResult};

// Helper function to create test JSON files
fn create_test_json_file(dir: &Path, filename: &str, screenshot_url: &str) -> Result<PathBuf, Box<dyn Error>> {
...
fn create_json_with_empty_screenshot(dir: &Path, filename: &str) -> Result<PathBuf, Box<dyn Error>> {
    let json_path = dir.join(filename);
    let json_content = json!({
        "id": "test-id",
        "title": "Test Title",
...
fn test_process_json_file_success() {
    ...
    let mock = server.mock("GET", "/test_image.png")
        .with_status(200)
    ...
    match result {
        FileProcessResult::Success => {
            ...
            assert!(screenshot.starts_with("data:image/png;base64,"), "Screenshot should be a PNG data URL");
...
fn test_process_json_file_empty_screenshot() {
    ...
    let json_path = create_json_with_empty_screenshot(&input_dir, "test_empty_screenshot.json").unwrap();
    let http_client = reqwest::blocking::Client::new();
...
fn test_process_json_file_null_screenshot() {
    ...
    let json_path = create_json_with_null_screenshot(&input_dir, "test_null_screenshot.json").unwrap();
    let http_client = reqwest::blocking::Client::new();
...
fn test_process_json_file_invalid_url() {
    ...
    let json_path = create_test_json_file(&input_dir, "test_invalid_url.json", "not_a_valid_url").unwrap();
    let http_client = reqwest::blocking::Client::new();
...
fn test_process_json_file_server_error() {
    ...
    let mut server = MockServer::new();
    let mock = server.mock("GET", "/non_existent.png")
        .with_status(404)
        .with_body("Not Found")
        .create();
    ...
    let json_path = create_test_json_file(&input_dir, "test_server_error.json", &image_url).unwrap();
    let http_client = reqwest::blocking::Client::new();
...
fn test_process_json_file_different_image_types() {
    ...
    // Test PNG processing
    {
        let png_mock = server.mock("GET", "/test_png.png")
            .with_status(200)
            .with_header("content-type", "image/png")
            .with_body(&png_image)
            .create();
    ...
    {
        let jpg_mock = server.mock("GET", "/test_jpg.jpg")
            .with_status(200)
        ...
        match result {
            FileProcessResult::Success => {
                ...
                let screenshot = json_value["screenshot"].as_str().unwrap();
                assert!(screenshot.starts_with("data:image/jpeg;base64,"), "Screenshot should be a JPEG data URL");
...
fn test_url_filename_extraction() {
    ...
    // Set up a mock HTTP server
    let mut server = MockServer::new();
    let mock = server.mock("GET", "/path/to/complex_filename.png")
        .with_status(200)
        .with_header("content-type", "image/png")
        .with_body(&test_image)
        .create();
    ...
    let json_path = create_test_json_file(&input_dir, "test_complex_path.json", &image_url).unwrap();
    ...
    match result {
        FileProcessResult::Success => {
            ...
            assert!(image_file_path.exists(), "Image file should exist with correct name extracted from URL");
            
            // Verify the content is correct
            let saved_image = fs::read(&image_file_path).unwrap();
            assert_eq!(saved_image, test_image, "Saved image content should match the original");
...
fn test_url_with_query_params() {
    ...
    // Set up a mock HTTP server
    let mut server = MockServer::new();
    let mock = server.mock("GET", "/image_with_params.png")
        .with_status(200)
        .with_header("content-type", "image/png")
        .with_body(&test_image)
    ...
    let image_url = format!("{}/image_with_params.png?width=800&height=600&token=abc123", server.url());
    let json_path = create_test_json_file(&input_dir, "test_query_params.json", &image_url).unwrap();
    ...
    match result {
        FileProcessResult::Success => {
            ...
            assert!(image_file_path.exists(), "Image file should exist with correct name extracted from URL (without query params)");
            mock.assert();
        },
        _ => panic!("Expected Success but got: {:?}", result),
    }
...
fn test_malformed_json() {
    ...
    fs::write(&json_path, r#"{ "id": "test-id", "screenshot": "http://example.com/image.png" "#).unwrap();
    ...
    match result {
        FileProcessResult::Failed(_, error_msg) => {
            assert!(error_msg.contains("JSON Error") || error_msg.contains("Failed to parse JSON"), 
                   "Error should indicate JSON parsing issue, but got: {}", error_msg);
        },
...
fn test_non_utf8_filenames() {
    ...
    match result {
        FileProcessResult::Success => {
            ...
            assert!(base64_json_path.exists(), "Base64 JSON file should exist with special characters in filename");
